...
class TowerEntity: GKEntity {
    ...
    init(towerType: TowerType) {
        ...
        shadowComponent = ShadowComponent(size: shadowSize,
                                          offset: CGPoint(x: 0.0, y: -textureSize.height / 2 + shadowSize.height / 2))
        addComponent(shadowComponent)
        // 加入动画
        animationComponent = AnimationComponent(node: spriteComponent.node, textureSize: textureSize,
